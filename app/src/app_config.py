...
class Config:
    ...
    G_APP_DESCRIPTION = """
		This application is a way to learn photogrammetry concepts and put them into practice. The application will provide a user with an easy way to perform a number of functions, ranging from data
		acquisition to 3D reconstruction. 
	"""
    G_APP_USAGE = """
	AP 3D Reconstruction <command> [<args>]

	    <command>               <description>
	    dlt                     Performs Direct Linear Transform (DLT) on the input data. File must contain image points, then object points. Output file will contain calibration, rotation and translation matrices.
